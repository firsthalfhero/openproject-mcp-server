...
from models import ProjectCreateRequest, ProjectUpdateRequest, WorkPackageCreateRequest, WorkPackageRelationCreateRequest
...
async def create_project(
    ...
    Args:
        ...
        parent_id: ID of the parent project to nest this project under (optional)
        status: Project status, one of on_track | off_track | at_risk |
            not_started | finished | discontinued (optional)
        identifier: URL-safe project identifier (optional; auto-generated from
            name when omitted, with numeric suffixes on collision)
    ...
    try:
        ...
        # Validate status against the allowed set, if provided.
        ...
        # Use the explicit identifier if given, otherwise auto-generate.
        ...
        # Attempt creation, auto-suffixing the identifier on collision (only
        # when it was auto-generated). Capped to avoid an unbounded loop.
        max_attempts = 1 if user_supplied_identifier else 10
        last_error: Optional[OpenProjectAPIError] = None
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                last_error = e
                # Retry with a new suffix only on identifier collisions we own.
                if _is_identifier_taken_error(e) and not user_supplied_identifier:
                    continue
                raise

        # Exhausted suffix attempts.
...
async def update_project(
    ...
    Args:
        ...
        parent_id: ID of the parent project to move this project under (optional)
        status: New project status, one of on_track | off_track | at_risk |
            not_started | finished | discontinued (optional)
